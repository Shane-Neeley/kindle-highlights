...
Python 3.13 tool that scrapes Kindle highlights from Amazon's notebook page using Playwright for browser automation and BeautifulSoup for HTML parsing. The repo is public; store personal notes or temporary findings in `docs/scratchpad/` (gitignored) instead of committing them.
...
Code lives directly under `src/` for both the CLI and FastAPI service.

### Module Structure

**`src/parser.py`** - HTML parsing and data models
...
**`src/main.py`** - CLI interface (argparse)
**`src/app.py`** - FastAPI service entrypoint wrapping the scraper
